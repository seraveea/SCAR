...
def evaluation(
    ...
    for p in predictions:
        ...
        if  not isinstance(answer[0], str):
            pred_range = process_numerical_answer(pred)
            answer_range = [float(a) for a in answer[0]['range']]
            quantity_pred.append(pred_range)
            quantity_answer.append(answer_range)
        elif time_instance(answer):
            time_pred.append(pred)
            time_answer.append(answer)
        else:
    ...
    # print("number of time questions:", str(len(time_pred)))
    # print("number of numerical questions:", str(len(quantity_pred)))
...
def get_results(
    ...
    print(f"sum of score_time: {sum(score_time)}, len of score_time: {len(score_time)}")
    final_score_quantity = safe_division(sum(score_quantity), len(score_quantity))
    print(f"sum of score_quantity: {sum(score_quantity)}, len of score_quantity: {len(score_quantity)}")
    final_score_entity = safe_division(sum(score_entity), len(score_entity))
    print(f"sum of score_entity: {sum(score_entity)}, len of score_entity: {len(score_entity)}")
...
def evaluate_infoseek_full(
    ...
    qid2examples: List[Dict[str, Dict[str, Any]]],
        ...
        infoseek_score = []
        # run two times, once on unseen question, once on unseen entity
...
def prepare_qid2example(
    ...
    for qtype in reference_qtype:
        ...
        qid2qtype[qid] = qtype["data_split"]
