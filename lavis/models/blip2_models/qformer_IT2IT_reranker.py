...
@registry.register_model("qformer_IT2IT_reranker")
class QformerIT2ITReranker(Blip2Base):
    ...
    def __init__(
        ...
        # --- 手动修 bias，防止和 checkpoint 维度不一致 ---
        # bias = self.Qformer.cls.predictions.bias
        # vocab_size = len(self.tokenizer)

        # if bias.shape[0] != vocab_size:
        #     new_bias = torch.zeros(vocab_size, dtype=bias.dtype, device="cpu")  # same dtype/device
        #     # 把原来的 30522 复制进去，剩下的新 token 位置初始化为 0
        #     new_bias[: bias.shape[0]] = bias
        #     self.Qformer.cls.predictions.bias = torch.nn.Parameter(new_bias)

        # print("after manual fix, decoder.bias shape:", self.Qformer.cls.predictions.bias.shape) 30523
    ...
    def forward(self, samples):
        ...
        query_attention_mask = torch.cat([query_atts, question_tokens.attention_mask], dim=1)
        query_fusion_output = self.Qformer.bert(
        ...
        query_fusion_feats = F.normalize(
            query_fusion_output.last_hidden_state[:, : query_tokens.size(1), :], dim=-1
        )
        ...
        # positive_fusion_feats = F.normalize(
        #     self.vision_proj(positive_fusion_output.last_hidden_state[:, : query_tokens.size(1), :]), dim=-1
        # )
        ...
        negative_fusion_output = self.Qformer.bert(
            ...
            return_dict=True,
        ) # [batch_size * neg_num, len(query_tokens) + max_txt_len, hidden_size]
        ...
        candidate_fusion_feats = F.normalize(candidate_fusion_tokens, dim=-1)
        
        # query token 2 candidate token -> max sum average
        #[bs, 1, 32, 256] [bs, 1 + neg_num, 32, 256]
        sim_q2c_token2token = torch.matmul(
            query_fusion_feats.unsqueeze(1), candidate_fusion_feats.permute(0, 1, 3, 2)
        )#[batch_size, 1 + neg_num, 32, 32]
        
        sim_q2c_querytoken, _ = sim_q2c_token2token.max(-1) #[batch_size, 1 + neg_num, 32]
        query_token_length = sim_q2c_querytoken.size(-1)
        sim_q2c = sim_q2c_querytoken.sum(-1) / query_token_length #[batch_size, 1 + neg_num]
    ...
    def extract_features(self, samples, mode="multimodal"):
        ...
        if mode == "image":
            ...
            text = self.tokenizer(caption, return_tensors="pt", padding='max_length', truncation=True, max_length=self.max_txt_len).to(self.device)
            ...
            multimodal_embeds = F.normalize(fusion_output.last_hidden_state[:, : query_tokens.size(1), :])
            ...
            multimodal_embeds = F.normalize(
                fusion_output.last_hidden_state[:, : query_tokens.size(1), :], dim=-1
            )
