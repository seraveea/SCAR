...
@registry.register_model("qformer_T2T_reranker")
class QformerT2TReranker(Blip2Base):
    ...
    def forward(self, samples):
        ...
        positive_output = self.Qformer.bert(
            positive_caption_tokens.input_ids,
            attention_mask=positive_caption_tokens.attention_mask,
            return_dict=True,
        )
        ...
        negative_output = self.Qformer.bert(
            ...
            attention_mask=negative_caption_tokens.attention_mask,
            return_dict=True,
        ) # [batch_size * neg_num, len(query_tokens) + max_txt_len, hidden_size]
        negative_cls = negative_output.last_hidden_state[:, 0, :].view(bs, -1, negative_output.last_hidden_state.size(-1)) # [batch_size, neg_num, hidden_size]
        # concat the positive with negative
        positive_cls = positive_output.last_hidden_state[:, 0, :].unsqueeze(1)
        concatenated_cls = torch.cat([positive_cls, negative_cls], dim=1)
        ...
        # query 2 candidate 
        #[bs, 1, 256] [bs, 1 + neg_num, 256] 

        sim_q2c = torch.matmul(
            query_feats.unsqueeze(1), candidate_feats.permute(0, 2, 1)
        ).squeeze() #[batch_size, 1 + neg_num]
        

        assert sim_q2c.size(-1) == candidate_feats.size(1)
